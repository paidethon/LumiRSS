/** RegisterScreen — 公开注册页（/register，未登录可达；P0-02）。
 *
 * 服务端权威：实例策略关闭（allow_public_registration=false）时
 * POST /auth/register 返回 403 registration_disabled（统一拒绝，不做
 * 用户名探测）——本页把它渲染成 F019 的诚实状态：「本实例未开放注册」
 * + 邀请激活入口（/activate）。前端隐藏按钮从来不是闸门，服务端才是。
 *
 * 表单：用户名（3–32 位小写字母/数字/-/_，字母或数字开头，客户端
 * 预校验、服务端仍权威）+ 可选显示名 + 密码/确认；
 * - F016 密码强度：纯本地三档（长度 × 字符类别多样性），不发任何
 *   网络请求、不上传任何派生值；
 * - F017 Caps Lock：getModifierState('CapsLock') 实时提示（密码与
 *   确认两个输入框都覆盖）。
 *
 * 服务端错误逐类映射为诚实中文文案：username_taken / weak_password /
 * invalid_username / registration_disabled；网络错误绝不误导为
 * 「用户名被占用」。
 *
 * F020 注册成功自动登录：服务端已 mint 会话 Cookie，身份仍由随后的
 * GET /auth/session 服务端核实（与 /auth/activate 同契约）；O157 先清
 * 上一账号足迹再翻门；带合法 ?next=（F015 同源路径校验）时回到认证
 * 前目标，否则进应用首页。
 */

import { useState } from 'react'
import { useQueryClient } from '@tanstack/react-query'
import { Eye, EyeOff, Lock, UserPlus } from 'lucide-react'
import {
  ApiError,
  getAuthSession,
  registerAccount,
} from '../api/client'
import { identityFromSession, useAuthStore } from '../store/auth'
import { resetAccountState } from '../lib/auth-reset'
import {
  isSafeAuthRedirectPath,
  navigateAppRoute,
  navigateToPath,
  readAppRoute,
  readAuthRedirectTarget,
} from '../lib/app-route'
import {
  passwordStrength,
  STRENGTH_TEXT,
  type PasswordStrength,
} from '../lib/password-strength'
import { Button } from './ui/Button'

/** 与 BFF USERNAME_RE 同规则：3–32 位，小写字母/数字/`-`/`_`，字母或数字开头。 */
const USERNAME_PATTERN = /^[a-z0-9][a-z0-9_-]{2,31}$/
const MIN_PASSWORD_LENGTH = 8

const STRENGTH_COLOR: Record<PasswordStrength, string> = {
  weak: 'bg-[var(--lumi-danger)]',
  medium: 'bg-[var(--lumi-text-tertiary)]',
  strong: 'bg-[var(--lumi-accent)]',
}

function usernameIssue(value: string): string | null {
  if (value === '') return '请输入用户名。'
  if (!USERNAME_PATTERN.test(value)) {
    return '用户名需 3–32 位，仅限小写字母、数字、- 或 _，并以字母或数字开头。'
  }
  return null
}

function passwordIssue(value: string): string | null {
  if (value === '') return '请输入密码。'
  if (value.length < MIN_PASSWORD_LENGTH) {
    return `密码至少 ${MIN_PASSWORD_LENGTH} 位。`
  }
  return null
}

/** F017：Caps Lock 实时提示状态（密码/确认输入框共用一条提示）。
 * getModifierState 异常（极端环境）时安静降级为不提示。 */
function capsLockFromEvent(event: React.KeyboardEvent): boolean {
  try {
    return event.getModifierState('CapsLock')
  } catch {
    return false
  }
}

type FieldErrors = { username?: string; password?: string; confirm?: string }

export default function RegisterScreen() {
  const queryClient = useQueryClient()
  const [username, setUsername] = useState('')
  const [displayName, setDisplayName] = useState('')
  const [password, setPassword] = useState('')
  const [confirm, setConfirm] = useState('')
  const [reveal, setReveal] = useState(false)
  const [capsLockOn, setCapsLockOn] = useState(false)
  const [pending, setPending] = useState(false)
  const [fieldErrors, setFieldErrors] = useState<FieldErrors>({})
  const [formError, setFormError] = useState<string | null>(null)
  const [closed, setClosed] = useState(false)

  const strength = passwordStrength(password)

  /** F020 成功路径：服务端已发会话 → 身份服务端核实 → 清足迹 → 翻门 →
   * 回 ?next=（F015 校验）或应用首页。 */
  async function finishRegistration() {
    let identity = null
    try {
      identity = identityFromSession(await getAuthSession())
    } catch {
      identity = null
    }
    resetAccountState(queryClient)
    useAuthStore.getState().setIdentity(identity)
    const next = readAuthRedirectTarget()
    if (next !== null && isSafeAuthRedirectPath(next)) {
      navigateToPath(next, true)
    } else if (readAppRoute() !== 'app') {
      navigateAppRoute('app', true)
    }
    useAuthStore.getState().setStatus('authenticated')
  }

  async function handleSubmit(event: React.FormEvent) {
    event.preventDefault()
    if (pending) return
    const issues: FieldErrors = {}
    const usernameError = usernameIssue(username)
    if (usernameError !== null) issues.username = usernameError
    const passwordError = passwordIssue(password)
    if (passwordError !== null) issues.password = passwordError
    if (confirm !== password) issues.confirm = '两次输入的密码不一致。'
    setFieldErrors(issues)
    setFormError(null)
    if (Object.keys(issues).length > 0) return

    setPending(true)
    try {
      const status = await registerAccount({
        username,
        password,
        displayName: displayName !== '' ? displayName : null,
      })
      if (status.authenticated) {
        await finishRegistration()
        return
      }
      setFormError('注册未完成，请稍后重试。')
    } catch (error) {
      if (error instanceof ApiError && error.type === 'registration_disabled') {
        // F019：实例未开放注册 —— 诚实告知 + 邀请激活入口。
        setClosed(true)
      } else if (error instanceof ApiError && error.type === 'username_taken') {
        setFieldErrors({ username: '该用户名已被使用，请换一个。' })
      } else if (error instanceof ApiError && error.type === 'weak_password') {
        setFieldErrors({ password: `密码强度不足：至少 ${MIN_PASSWORD_LENGTH} 位，建议混用字符种类。` })
      } else if (error instanceof ApiError && error.type === 'invalid_username') {
        setFieldErrors({ username: '用户名格式不正确：3–32 位，仅限小写字母、数字、- 或 _，以字母或数字开头。' })
      } else if (error instanceof ApiError && error.type === 'network_error') {
        setFormError('网络不可用 —— 请检查网络连接后重试。')
      } else if (error instanceof ApiError) {
        setFormError(error.message)
      } else {
        setFormError('注册失败，请稍后重试。')
      }
    } finally {
      setPending(false)
    }
  }

  const capsLockHandlers = {
    onKeyDown: (event: React.KeyboardEvent) => setCapsLockOn(capsLockFromEvent(event)),
    onKeyUp: (event: React.KeyboardEvent) => setCapsLockOn(capsLockFromEvent(event)),
    onBlur: () => setCapsLockOn(false),
  }

  const inputClassName =
    'min-h-11 w-full rounded-[var(--lumi-radius-md)] border border-[var(--lumi-border)] bg-[var(--lumi-surface)] px-3 text-base text-[var(--lumi-text-primary)] placeholder:text-[var(--lumi-text-tertiary)] transition-colors duration-[var(--lumi-motion-fast)] hover:border-[var(--lumi-text-tertiary)] focus-visible outline-2 -outline-offset-1 outline-[var(--lumi-focus-ring)] disabled:opacity-50'

  return (
    <div className="flex min-h-dvh items-center justify-center bg-[var(--lumi-canvas)] px-6 py-[max(1.5rem,env(safe-area-inset-top))] pb-[max(1.5rem,env(safe-area-inset-bottom))]">
      <div className="w-full max-w-sm" data-testid="register-screen">
        <div className="mb-8 flex flex-col items-center gap-3 text-center">
          <img src="/icons/lumirss-icon.svg" alt="" className="size-14" decoding="async" />
          <div>
            <h1 className="text-xl font-semibold tracking-tight text-[var(--lumi-text-primary)]">
              创建账号
            </h1>
            <p className="mt-1 text-sm text-[var(--lumi-text-secondary)]">
              流光阅源 · 注册后立即使用
            </p>
          </div>
        </div>

        {closed ? (
          <div
            className="flex flex-col items-center gap-4 rounded-[var(--lumi-radius-md)] border border-[var(--lumi-border)] bg-[var(--lumi-surface)] p-5 text-center"
            data-testid="register-closed"
          >
            <Lock aria-hidden className="size-6 text-[var(--lumi-text-tertiary)]" />
            <p role="alert" className="text-sm font-medium text-[var(--lumi-text-primary)]">
              本实例未开放注册
            </p>
            <p className="text-xs leading-relaxed text-[var(--lumi-text-secondary)]">
              运营者已关闭公开注册。如果你持有邀请链接，仍可以激活加入。
            </p>
            <Button
              variant="secondary"
              onClick={() => navigateAppRoute('activate')}
              data-testid="register-closed-activate-link"
            >
              前往邀请激活
            </Button>
            <button
              type="button"
              onClick={() => navigateAppRoute('app')}
              className="min-h-11 text-xs text-[var(--lumi-text-tertiary)] underline-offset-2 hover:underline focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-[var(--lumi-focus-ring)]"
            >
              返回登录
            </button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="flex flex-col gap-3" noValidate>
            <div>
              <label
                htmlFor="register-username"
                className="mb-1.5 block text-sm font-medium text-[var(--lumi-text-primary)]"
              >
                用户名
              </label>
              <input
                id="register-username"
                type="text"
                value={username}
                onChange={(e) => setUsername(e.target.value.toLowerCase())}
                autoComplete="username"
                autoCapitalize="none"
                autoCorrect="off"
                spellCheck={false}
                enterKeyHint="next"
                inputMode="text"
                maxLength={32}
                disabled={pending}
                aria-invalid={fieldErrors.username !== undefined || undefined}
                aria-describedby={
                  [
                    fieldErrors.username !== undefined ? 'register-username-error' : null,
                    'register-username-hint',
                  ]
                    .filter(Boolean)
                    .join(' ') || undefined
                }
                className={inputClassName}
                placeholder="小写字母、数字、- 或 _"
              />
              <p
                id="register-username-hint"
                className="mt-1 text-xs leading-relaxed text-[var(--lumi-text-tertiary)]"
              >
                3–32 位，小写字母 / 数字 / - / _，字母或数字开头。
              </p>
              {fieldErrors.username !== undefined && (
                <p
                  id="register-username-error"
                  className="mt-1 text-xs leading-relaxed text-[var(--lumi-danger)]"
                >
                  {fieldErrors.username}
                </p>
              )}
            </div>

            <div>
              <label
                htmlFor="register-display-name"
                className="mb-1.5 block text-sm font-medium text-[var(--lumi-text-primary)]"
              >
                显示名（可选）
              </label>
              <input
                id="register-display-name"
                type="text"
                value={displayName}
                onChange={(e) => setDisplayName(e.target.value)}
                autoComplete="nickname"
                maxLength={64}
                disabled={pending}
                className={inputClassName}
                placeholder="昵称或真名"
              />
            </div>

            <div>
              <label
                htmlFor="register-password"
                className="mb-1.5 block text-sm font-medium text-[var(--lumi-text-primary)]"
              >
                密码
              </label>
              <div className="relative">
                <input
                  id="register-password"
                  type={reveal ? 'text' : 'password'}
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  autoComplete="new-password"
                  disabled={pending}
                  aria-invalid={fieldErrors.password !== undefined || undefined}
                  aria-describedby={
                    [
                      fieldErrors.password !== undefined ? 'register-password-error' : null,
                      password !== '' ? 'register-password-strength' : null,
                      capsLockOn ? 'register-capslock-hint' : null,
                    ]
                      .filter(Boolean)
                      .join(' ') || undefined
                  }
                  {...capsLockHandlers}
                  className={`${inputClassName} pr-11`}
                  placeholder="至少 8 位"
                />
                <button
                  type="button"
                  onClick={() => setReveal((v) => !v)}
                  aria-label={reveal ? '隐藏密码' : '显示密码'}
                  className="absolute inset-y-0 right-0 flex w-11 items-center justify-center rounded-[var(--lumi-radius-md)] text-[var(--lumi-text-tertiary)] transition-colors duration-[var(--lumi-motion-fast)] hover:text-[var(--lumi-text-primary)] focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-[var(--lumi-focus-ring)]"
                >
                  {reveal ? <EyeOff aria-hidden className="size-4" /> : <Eye aria-hidden className="size-4" />}
                </button>
              </div>
              {strength !== null && (
                <div id="register-password-strength" data-testid="password-strength" data-strength={strength}>
                  <div className="mt-1.5 flex gap-1" aria-hidden>
                    {(['weak', 'medium', 'strong'] as const).map((tier) => (
                      <span
                        key={tier}
                        className={`h-1 flex-1 rounded-full transition-colors duration-[var(--lumi-motion-fast)] ${
                          tier === strength ? STRENGTH_COLOR[strength] : 'bg-[var(--lumi-separator)]'
                        }`}
                      />
                    ))}
                  </div>
                  <p className="mt-1 text-xs leading-relaxed text-[var(--lumi-text-secondary)]">
                    {STRENGTH_TEXT[strength]}
                  </p>
                </div>
              )}
              {fieldErrors.password !== undefined && (
                <p
                  id="register-password-error"
                  className="mt-1 text-xs leading-relaxed text-[var(--lumi-danger)]"
                >
                  {fieldErrors.password}
                </p>
              )}
            </div>

            <div>
              <label
                htmlFor="register-confirm"
                className="mb-1.5 block text-sm font-medium text-[var(--lumi-text-primary)]"
              >
                确认密码
              </label>
              <input
                id="register-confirm"
                type={reveal ? 'text' : 'password'}
                value={confirm}
                onChange={(e) => setConfirm(e.target.value)}
                autoComplete="new-password"
                disabled={pending}
                aria-invalid={fieldErrors.confirm !== undefined || undefined}
                aria-describedby={
                  [
                    fieldErrors.confirm !== undefined ? 'register-confirm-error' : null,
                    capsLockOn ? 'register-capslock-hint' : null,
                  ]
                    .filter(Boolean)
                    .join(' ') || undefined
                }
                {...capsLockHandlers}
                className={inputClassName}
                placeholder="再输入一次"
              />
              {fieldErrors.confirm !== undefined && (
                <p
                  id="register-confirm-error"
                  className="mt-1 text-xs leading-relaxed text-[var(--lumi-danger)]"
                >
                  {fieldErrors.confirm}
                </p>
              )}
            </div>

            {capsLockOn && (
              <p
                id="register-capslock-hint"
                role="status"
                data-testid="capslock-hint"
                className="text-xs leading-relaxed text-[var(--lumi-text-secondary)]"
              >
                大写锁定（Caps Lock）已开启。
              </p>
            )}

            {formError !== null && (
              <p role="alert" className="text-xs leading-relaxed text-[var(--lumi-danger)]">
                {formError}
              </p>
            )}

            <Button
              type="submit"
              variant="primary"
              className="min-h-11 w-full"
              disabled={pending || username.trim().length === 0 || password.length === 0 || confirm.length === 0}
            >
              <UserPlus aria-hidden className="size-4" />
              {pending ? '注册中…' : '注册并进入'}
            </Button>

            <p className="mt-1 text-center text-xs leading-relaxed text-[var(--lumi-text-tertiary)]">
              注册后你将拥有独立的订阅与 RSS 账号绑定，不与他人共用凭据。
            </p>

            <button
              type="button"
              onClick={() => {
                // 回登录页时把合法 ?next= 带上（登录成功后仍回认证前目标）。
                const next = readAuthRedirectTarget()
                if (next !== null) navigateToPath(`/?next=${encodeURIComponent(next)}`)
                else navigateAppRoute('app')
              }}
              data-testid="register-back-login"
              className="mt-1 flex min-h-11 items-center justify-center text-sm text-[var(--lumi-accent-text)] underline-offset-2 hover:underline focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-[var(--lumi-focus-ring)]"
            >
              已有账号？返回登录
            </button>
          </form>
        )}
      </div>
    </div>
  )
}
