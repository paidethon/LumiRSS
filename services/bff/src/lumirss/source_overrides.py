...
def _row_to_dict(row: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "language": _nullable("language"),
        "unreadAlertThreshold": _nullable("unread_alert_threshold"),
        "syncPriority": _nullable("sync_priority"),
...
class SourceOverrideStore:
    ...
    async def list_overrides(self) -> list[dict[str, Any]]:
        ...
        rows = await self._db.fetch_all(
            "SELECT feed_url, hidden_until, show_from, stale_alert_hours, extract_policy, reader_style_json, ai_disabled, mute_windows_json, encoding_override, language, unread_alert_threshold, sync_priority, updated_at FROM source_overrides WHERE hidden_until IS NOT NULL OR show_from IS NOT NULL OR stale_alert_hours IS NOT NULL OR mute_windows_json IS NOT NULL OR encoding_override IS NOT NULL OR language IS NOT NULL OR unread_alert_threshold IS NOT NULL OR sync_priority IS NOT NULL ORDER BY updated_at DESC"
        )
    ...
    async def get_override(self, feed_url: str) -> dict[str, Any] | None:
        ...
        row = await self._db.fetch_one(
            "SELECT feed_url, hidden_until, show_from, stale_alert_hours, extract_policy, reader_style_json, ai_disabled, mute_windows_json, encoding_override, language, unread_alert_threshold, sync_priority, updated_at FROM source_overrides WHERE feed_url = ?",
            (feed_url,),
        )
        if row is None:
            return None
    ...
    async def set_fields(
        ...
        *,
        ...
        if hidden_value is None and show_value is None and stale_value is None:
            ...
            # 等其它维度仍有值时保留）。
            row = await self._db.fetch_one(
                "SELECT ai_disabled, extract_policy, reader_style_json, mute_windows_json, encoding_override, language, unread_alert_threshold, sync_priority FROM source_overrides WHERE feed_url = ?",
                (feed_url,),
            )
            keep = (
                row is not None
