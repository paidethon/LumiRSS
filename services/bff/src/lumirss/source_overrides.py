...
class SourceOverrideStore:
    ...
    async def list_overrides(self) -> list[dict[str, Any]]:
        ...
        rows = await self._db.fetch_all(
            "SELECT feed_url, hidden_until, show_from, stale_alert_hours, extract_policy, reader_style_json, ai_disabled, mute_windows_json, encoding_override, attention_level, refresh_advisory, language, unread_alert_threshold, sync_priority, translation_policy, updated_at FROM source_overrides WHERE hidden_until IS NOT NULL OR show_from IS NOT NULL OR stale_alert_hours IS NOT NULL OR mute_windows_json IS NOT NULL OR encoding_override IS NOT NULL OR attention_level IS NOT NULL OR refresh_advisory IS NOT NULL OR language IS NOT NULL OR unread_alert_threshold IS NOT NULL OR sync_priority IS NOT NULL OR translation_policy IS NOT NULL ORDER BY updated_at DESC"
        )
    ...
    async def get_override(self, feed_url: str) -> dict[str, Any] | None:
        ...
        row = await self._db.fetch_one(
            "SELECT feed_url, hidden_until, show_from, stale_alert_hours, extract_policy, reader_style_json, ai_disabled, mute_windows_json, encoding_override, attention_level, refresh_advisory, language, unread_alert_threshold, sync_priority, translation_policy, updated_at FROM source_overrides WHERE feed_url = ?",
            (feed_url,),
        )
        if row is None:
            return None
