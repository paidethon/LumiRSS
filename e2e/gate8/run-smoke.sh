...
internal() { # internal <path> [curl-args...] — internal-token bff call
  local path="$1"; shift
  curl -sS -H "X-Lumi-Token: $INTERNAL_TOKEN" "$@" "$BFF$path"
}
...
async def main():
    ...
    await store.set_password("e2e-login-" + "pw")
...
smoke_read_later_server_side() { # 2. read-later 服务端时间线
  local ref
  ref=$(internal '/api/v1/search?q=e2e&limit=1' | python3 -c 'import json,sys; d=json.load(sys.stdin); r=(d.get("items") or [{}])[0].get("entryRef",""); print(r)' 2>/dev/null || true)
  if [[ -z "$ref" ]]; then
...
smoke_mixed_workspace() { # 3. 混合 workspace
  local ws
  ws=$(api POST /api/v1/workspaces -H 'content-type: application/json' -d '{"name": "e2e-mixed"}')
  local ws_id
  ...
  local kinds
  kinds=$(api "/api/v1/workspaces/$ws_id/contents" | python3 -c 'import json,sys; print(",".join(i["kind"] for i in json.load(sys.stdin)["items"]))')
...
smoke_clip_pipeline() { # 4. 剪藏服务端提取 + 恶意 HTML 清理
  ...
  api POST /api/v1/library/clips -H 'content-type: application/json' \
    -d '{"url": "http://fixtures/malicious.html"}' >/dev/null
  local stored_ok=1
  api GET '/api/v1/library/clips?limit=10' | grep -q '受控剪藏正文' || stored_ok=0
  [[ "$stored_ok" == "1" ]]
...
smoke_snapshot_offline() { # 5. snapshot 生产镜像创建 + 离线内容
  ...
  local first_asset
  first_asset=$(internal /api/v1/library/snapshots | python3 -c 'import json,sys; items=json.load(sys.stdin)["items"]; print(items[0]["uuid"] if items else "")')
  [[ -n "$first_asset" ]] || { fail "05 snapshot list empty"; return; }
  $COMPOSE exec -T bff grep -q '受控剪藏正文' "/data/library/assets/$first_asset.page.html"
...
smoke_freshrss_subscription() { # 7. FreshRSS 订阅 API Source 并抓到 entry
  ...
  src=$(api POST /api/v1/api-sources -H 'content-type: application/json' -d '{
    ...
    "itemsExpr": "items",
    "fieldMap": {"id": "url", "title": "title", "url": "url", "published": "published", "content": "summary"},
    "subscribe": true
  }')
  ...
  $COMPOSE exec -T lumirss-e2e-freshrss curl -sf "http://bff:8000$atom_path" | grep -q 'sqlite-vec'
  check "07 freshrss container fetches the generated atom (entry present)" $?
}

smoke_etag_stale() { # 8. 稳定 ETag/304
  local src atom_path etag code
  src=$(api POST /api/v1/api-sources -H 'content-type: application/json' -d '{
    "name": "e2e-etag",
    "endpoint": "http://fixtures/sample.json",
    "itemsExpr": "items",
  ...
  atom_path=$(echo "$src" | python3 -c 'import json,sys; print(json.load(sys.stdin)["atomPath"])')
  $COMPOSE exec -T bff curl -sf "http://127.0.0.1:8000$atom_path" >/dev/null
  etag=$($COMPOSE exec -T bff curl -sSI "http://127.0.0.1:8000$atom_path" | grep -i '^etag:' | tr -d '\r' | awk '{print $2}')
  [[ -n "$etag" ]] || { fail "08 no etag"; return; }
  code=$($COMPOSE exec -T bff curl -sS -o /dev/null -w '%{http_code}' -H "If-None-Match: $etag" "http://127.0.0.1:8000$atom_path")
  [[ "$code" == "304" ]]
  check "08 stable etag + reliable 304" $?
}

smoke_webhook_bearer() { # 9. session auth 下 mail webhook 通路
  local list path wrong
  list=$(api POST /api/v1/mail/bridge-lists -H 'content-type: application/json' \
    -d "{\"name\": \"e2e-list\", \"secret\": \"e2e-list-s3cret-0123456789abcdef\"}")
  ...
  if [[ -n "$path" ]]; then
    $COMPOSE exec -T lumirss-e2e-freshrss curl -sf "http://bff:8000$path" | grep -q '<feed'
...
async def main():
    ...
    assert report["added"] == 4, report  # duplicate content = 2 identities
